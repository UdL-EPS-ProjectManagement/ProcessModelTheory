...
[.cue]
****
In terms of features, the iterative model has better *adaptation to change* than the waterfall 
model. In this model, there is lot of documentation at global scale (could be a benefit) and in 
each iteration (disadvantage). This model is heavy due to the iterations planning and complete 
development schemes to perform in each iteration. 
****
